...
| D-24 | Added, not in legacy: admin guards and switch-admin's functions. Guard **A** (`requireAdmin`, `src/cloud/guards.ts`) = guard S, then the caller's *current* row must be an enabled admin (`staffType` admin, staff/admin `appType`), else `141 ADMIN_REQUIRED`. Applied to: `updateConfigs`; `addUser` when `staffType` is set or `appType` holds staff/admin; `editUser` when the target is staff-tagged (any `staffType`, or a staff/admin `appType`), `staffType` changes, or staff/admin is added to `appType`; `deleteUsers` / `toggleEnableUsers` when any target is staff-tagged (checked for the whole batch first). New, all guard A: `signOutStaff({ userId })` destroys a staff account's sessions → `{ objectId, sessions }`; `removeStaff({ userId })` drops it from the `Staff` role, unsets `staffType`/`opsAccess`/`financeAccess`, removes staff/admin from `appType`, ends its sessions → `{ objectId, sessions }` (both refuse the caller `SELF_NOT_ALLOWED`, a non-staff target `NOT_STAFF_ACCOUNT`, a missing one `USER_DOES_NOT_EXISTS`); `recountRatings({ restaurantIds?, driverIds? })` (≤100 ids) recounts `rating`/`ratingTotal`/`reviews` (restaurant) or `driverRating`/`driverParams.{ratingTotal,reviews}` (driver) from the reviews left → `{ restaurants, drivers }` | Yes, intended: any Staff session could edit Config, give itself `Admin` through `editUser`, or rewrite/delete another staff account; only switch-admin's admin screens do these now (switch-dashboard's Configs and staff user edits were already admin-only in its UI; switch-ops refuses staff accounts and never sends `staffType` for others, so it is unaffected). Deactivating a staff member can now end their sessions, and deleting a review no longer leaves inflated totals. **Canary:** legacy has no guard and answers `141 Invalid function` to the three new functions, which switch-admin shows as "needs the new server" | `admin.test.ts`, registry test |
...
**switch-admin:** `loginStaff` (admins only, checked in the console), regions / categories / banners
(`City`, `Category`, `Ad` plain saves), promos (`uniquePromo`, `editPromo`, `assignPromo`,
`deletePromos`), push campaigns (`sendPush` + `Notification` log), team (`addUser`, `editUser`,
`toggleEnableUsers`, `deleteUsers`, `getUsers`, `setOpsAccess`, `setFinanceAccess`, `signOutStaff`,
`removeStaff`), reviews (`deleteReviews`, `recountRatings`), app settings (`updateConfigs`) —
every admin guard of D-24 must refuse an `ops` session and pass `admin`.
